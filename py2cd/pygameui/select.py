...
class SelectView(view.View):
    ...
    def layout(self):
        ...
        if self.scroll_view.hidden:
            self.frame.h = theme.aktive_szene.label_height
        else:
            self.frame.h = (self.top_label.frame.h +
                            self.scroll_view.frame.h - 1)

        self.disclosure.frame.w = theme.aktive_szene.label_height
        self.disclosure.frame.h = theme.aktive_szene.label_height
        self.disclosure.frame.right = self.scroll_view.frame.right

        self.top_label.frame.w = self.disclosure.frame.left
        self.top_label.frame.h = theme.aktive_szene.label_height
